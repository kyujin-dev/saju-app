...
const PROFILES = {
  ...
  가족: { 일간:12, 일지:12, 용신상보:30, 매트릭스:20, 십성:14, 조후강약:9,  연지:3,
         설명:'끊을 수 없는 관계라 8글자 전체의 충합과 상보를 본다' },
...
출력: { "요약": string, "얻는것": finding[], "내주는것": finding[], "A가할일": action[] }`,
    pick: (r, m) => ({ 점수: m.궁합.A입장, 용신상보: m.궁합.용신상보.A가받는것,
...
출력: { "요약": string, "얻는것": finding[], "내주는것": finding[], "B가할일": action[] }`,
    pick: (r, m) => ({ 점수: m.궁합.B입장, 용신상보: m.궁합.용신상보.B가받는것,
...
const PATTERN_T = {
  살인상생:'편관의 압박이 인성을 거쳐 나를 키우는 힘으로 바뀝니다. 스트레스가 배움과 자격으로 전환되는 구조라 조직·전문직에서 크게 쓰입니다.',
  관인상생:'정관과 인성이 이어져 자리와 배움이 서로를 밀어줍니다. 명예로운 직역에서 안정적으로 오르는 형태입니다.',
  식신제살:'식신이 칠살을 눌러 흉이 권위로 바뀝니다. 압박을 실력으로 제압하는 구조입니다.',
  제살태과:'살을 누르는 힘이 지나쳐 오히려 쓸 관이 남지 않습니다. 통제가 과해 기회를 놓치기 쉽습니다.',
  살중신경:'칠살은 무거운데 내 힘이 가볍습니다. 감당보다 버티기가 되기 쉬워, 인성으로 받쳐주는 것이 관건입니다.',
  관살혼잡:'정관과 편관이 섞여 있습니다. 자리와 압박이 뒤엉켜 방향이 흔들리기 쉽고, 하나로 정리될 때 풀립니다.',
  양인가살:'양인의 날카로움을 칠살이 다스립니다. 거칠지만 통제되면 큰 힘이 되는 구조입니다.',
  ...
  재생살:'재물이 칠살을 키웁니다. 돈 문제가 압박으로 되돌아오기 쉬워 관리가 필요합니다.',
...
function explain(kind, value, r) {
  ...
  switch (kind) {
    case '강약': return { 제목:'강약 — ' + value,
      뭐냐:'일간(나)이 얼마나 힘이 있는지를 봅니다. 인성과 비겁이 나를 돕는 편이고, 식상·재성·관성은 나를 쓰는 편입니다.',
      내경우: T_LEVEL[value] + (S ? ` 인비 세력이 ${S.allyPct}%로 나왔고, 판정 신뢰도는 ${S.confidence}입니다.` : '') };
    case '뿌리': return { 제목:'뿌리(통근) — ' + value,
      뭐냐:'천간의 글자가 지지에 뿌리를 내렸는지 봅니다. 뿌리가 없으면 겉만 있고 실속이 없다고 봅니다.',
      내경우: T_ROOT[value] + (r ? ` ${r.통근.일간.total}점이며 ${r.통근.일간.roots.map(x=>x.pos+x.ji).join('·')||'뿌리 없음'}에 자리합니다.` : '') };
    case '조합': return { 제목: value,
      뭐냐:'십성 두 가지가 만나 만드는 구조입니다. 낱개 십성보다 이 조합이 삶의 결을 더 잘 보여줍니다.',
      내경우: (PATTERN_T[value] || '') +
        (r ? (() => { const p = (r.패턴||[]).find(x => x.name === value);
          return p ? ` 이 사주에서는 ${p.force} 정도로 드러나며, ${p.구성}으로 성립합니다.` : ''; })() : '') };
    case '종격': return { 제목:'특수격 — ' + value,
      뭐냐:'한 기운이 압도적이라 억부(강약을 맞추는) 논리가 통하지 않는 격입니다. 거스르지 않고 따라가는 것이 관건입니다.',
      내경우: (T_JONG[value] || '') + (r ? ' ' + r.yongsin.primary.evidence : '') };
    case '격국': return { 제목:'격국 — ' + value + (r && r.격국.별칭 && r.격국.별칭!==value ? ' (' + r.격국.name + ')' : ''),
      뭐냐:'월지를 중심으로 이 사주가 어떤 유형인지 정합니다. 성격의 큰 틀이라고 보면 됩니다.',
      내경우: (T_GYEOK_ALT[value] || T_GYEOK[value] || '') + (r ? ' ' + (T_SUCCESS[r.성패.판정]||'') + ' ' + r.성패.근거 : '') +
        (r && r.격국.별칭 && r.격국.별칭 !== value
          ? ` 월지가 ${r.격국.월지십성}이라 ${r.격국.별칭}이라고도 부르며, ${T_GYEOK_ALT[r.격국.별칭]||''}` : '') +
        (r && r.격국.유파 ? ' ※ ' + r.격국.유파 : '') };
    case '용신': return { 제목:'용신 — ' + value,
      뭐냐:'사주에서 가장 필요한 기운입니다. 이 기운이 운에서 들어올 때 일이 풀린다고 봅니다.',
      내경우: (T_YONGSIN[value]||'') + (Y ? ' ' + Y.primary.evidence : '') +
        (r && r.groupPower[value] != null ? ` 다만 원국에 ${r.groupPower[value]}%뿐이라 운에서 채워져야 제 힘을 씁니다.` : '') };
    case '그릇': return { 제목:'그릇 — ' + value,
      뭐냐:'격의 크기입니다. 길흉이 아니라 담을 수 있는 양이고, 운이 받쳐줘야 실제로 쓰입니다.',
      내경우: (T_BAND[value]||'') + (r ? ' 근거: ' + r.격국고저.항목.slice(0,3).join(', ') : '') };
    ...
    case '십이운성': return { 제목:'십이운성 — ' + value,
      뭐냐:'천간이 각 지지에서 어느 기세에 있는지를 사람의 한살이(잉태→성장→쇠퇴→소멸)에 빗대 열두 단계로 나눈 것입니다. ' +
           '같은 글자라도 어느 지지에 앉느냐에 따라 힘이 달라집니다.',
      내경우: (UNSEONG_T[value] || '') +
